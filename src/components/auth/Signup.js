...
export default class Signup extends Component {
    ...
    return (
      <div className="SignupComponent container">
        <form className="form-signup" onSubmit={this.handleSignup}>
          ...
          <button className="SignupBtn btn btn-primary" type="submit" value="Submit">Sign Up</button>
        </form>
      </div>
    );
  }
  ...
  handleSignup = async (event) => {
    ...
    if (this.state.password !== this.state.confirmPassword) {
      ...
      try {
        const user = await Firebase.createUser(this.state.email, this.state.password);
